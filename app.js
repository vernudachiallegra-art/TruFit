...
// Session 10: the shopper's answers from the pre-chat style quiz. Sent along with
// every AI message so it can use them to RANK matches — never to override size
// or budget, which are enforced separately on the server no matter what's here.
let quizAnswers = {
  styles: [],
  shops: [],
  colours: []
...
/* ---------------- STYLE QUIZ (Session 10) ---------------- */
/*
  Shows once, on page load, before the shopper sees the chat. Three quick tap-through
  questions. Every step is skippable — nothing here is required to use TruFit.
  Answers get sent to the AI alongside every message (see sendToAI) purely to help it
  RANK matches ("this shopper likes pink and shops at Zara") — they can NEVER unlock
  an item that breaks the size or budget rules. Those are still checked separately,
  in code, on the server — same as before.
*/

// Reuses the same style words the matching engine already knows about, so quiz
// answers line up with real product data.
const QUIZ_STYLE_OPTIONS = STYLE_WORDS;

// A short, recognisable list of shops from the catalogue — not exhaustive, just
// enough to tap fast. "Or type your own" covers everything else.
const QUIZ_SHOP_OPTIONS = [
  "Adidas","Nike","ASOS","Zara","H&M","Vans",
  "Urban Outfitters","New Look","Topshop","Lululemon","Reformation","UGG"
];

const QUIZ_COLOUR_OPTIONS = [
  "Black","White","Pink","Red","Blue","Green","Beige/Neutral","Pastel","Bright/Colourful"
];

let quizStep = 0;
const QUIZ_TOTAL_STEPS = 3;

// Builds tappable chips into a container, toggling selection in/out of the given
// answers array. Re-used for all three questions.
function renderQuizChips(containerId, options, answersArray){
  ...
  options.forEach(opt=>{
    ...
    chip.onclick = ()=>{
      const idx = answersArray.indexOf(opt);
      if(idx>=0){
        answersArray.splice(idx,1);
        chip.classList.remove("selected");
      } else {
        answersArray.push(opt);
...
// Reads whatever's typed in a step's "or type your own" box and adds it into
// the matching answers array, so free-typed answers count just like tapped chips.
function captureQuizOther(inputId, answersArray){
  const input = document.getElementById(inputId);
  const val = input.value.trim();
  if(val.length>0 && !answersArray.includes(val)){
...
function initQuiz(){
  renderQuizChips("quizStyleChips", QUIZ_STYLE_OPTIONS.map(s=>s.charAt(0).toUpperCase()+s.slice(1)), quizAnswers.styles);
  renderQuizChips("quizShopChips", QUIZ_SHOP_OPTIONS, quizAnswers.shops);
  renderQuizChips("quizColourChips", QUIZ_COLOUR_OPTIONS, quizAnswers.colours);

  document.querySelectorAll(".quiz-next-btn:not(.quiz-finish-btn)").forEach(btn=>{
    btn.onclick = ()=>{
      const step = parseInt(btn.dataset.step);
      if(step===0) captureQuizOther("quizStyleOther", quizAnswers.styles);
      if(step===1) captureQuizOther("quizShopOther", quizAnswers.shops);
      goToQuizStep(step+1);
    };
  });

  document.querySelectorAll(".quiz-skip-btn").forEach(btn=>{
    btn.onclick = ()=>{
      const step = parseInt(btn.dataset.step);
      goToQuizStep(Math.min(step+1, QUIZ_TOTAL_STEPS-1));
      if(step===QUIZ_TOTAL_STEPS-1) closeQuiz();
    };
  });

  document.getElementById("quizFinishBtn").onclick = ()=>{
    captureQuizOther("quizColourOther", quizAnswers.colours);
    closeQuiz();
  };

  document.getElementById("quizSkipAll").onclick = ()=>{
    quizAnswers = { styles:[], shops:[], colours:[] };
    closeQuiz();
  };
}
...
// Turns the list of product names the AI sent back into the same clickable
// cards (link, price, Save button) that the old scripted matching used to build.
...
async function sendToAI(text){
  ...
  try{
    const response = await fetch("/.netlify/functions/chat", {
      ...
      body: JSON.stringify({ messages: conversationHistory, preferences: quizAnswers })
    });
    ...
    // Look up each product name the AI mentioned in the real catalogue, so we only
    // ever show items that actually exist (with their real link, price and size).
    if(data.products && data.products.length>0){
      const matched = data.products
        .map(name => PRODUCTS.find(p => p.name === name))
        .filter(Boolean)
        .map(p => Object.assign({}, p, { overBudget: (data.overBudget||[]).includes(p.name) }));
